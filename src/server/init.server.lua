...
-- Per-player save slots (science, unlocked parts, craft) persisted via DataStore.
local ok, err = pcall(function()
	require(script:WaitForChild("SaveServer")).start()
end)
if not ok then
	warn("[RocketSim] SaveServer failed to start: " .. tostring(err))
end
